...
public class HomeController {
	...
	public ModelAndView save(CustomerDTO customerDTO) throws Exception {
		...
		try {
			rest.exchange("http://localhost:9082/registration/save", HttpMethod.POST, request, String.class);
	...
	public ModelAndView createPackage(PackageDTO packageDTO) {
		...
		try {
			...
			rest.exchange("http://localhost:8088/admin/create", HttpMethod.POST, request, String.class);
	...
	public ModelAndView listPackages() {
		...
		ResponseEntity<List<PackageDTO>> response = rest.exchange("http://localhost:9082/admin/listPackage",
				HttpMethod.GET, req, new ParameterizedTypeReference<List<PackageDTO>>() {
				});
	...
	public ModelAndView listBookings() {
		...
		ResponseEntity<List<BookingDTO>> response = rest.exchange("http://localhost:9082/admin/listBooking",
				HttpMethod.GET, req, new ParameterizedTypeReference<List<BookingDTO>>() {
				});
	...
	public ModelAndView updatePackage(PackageDTO packageDTO) {
		...
		try {
			...
			rest.exchange("http://localhost:9082/admin/updatePackage", HttpMethod.PUT, request, String.class);
	...
	public ModelAndView deleteBooking(int bookingId) {
		...
		try {
			...
			rest.exchange("http://localhost:9082/admin/delete/" + bookingId, HttpMethod.DELETE, request, String.class);
	...
	public ModelAndView listPackagesForCustomer() {
		...
		ResponseEntity<List<PackageDTO>> response = rest.exchange("http://localhost:9082/customer/listPackage",
				HttpMethod.GET, req, new ParameterizedTypeReference<List<PackageDTO>>() {
				});
		mv.addObject("packList", response.getBody());
	...
	public ModelAndView myBookings(String userName) {
		...
		ResponseEntity<List<BookingDTO>> response = rest.exchange(
				"http://localhost:9082/customer/listBookings/" + userName, HttpMethod.GET, req,
	...
	public ModelAndView doBooking(BookingDTO bookingDTO) {
		...
		try {
			rest.exchange("http://localhost:9082/customer/book", HttpMethod.POST, request, String.class);

			mv.addObject(SUCCESS, "Package Reserved,Make Payment now");
	...
	public ModelAndView updateStatus(int bookingId) {
		...
		try {
			...
			rest.exchange("http://localhost:9082/v1/payment/" + bookingId, HttpMethod.GET, request, String.class);
